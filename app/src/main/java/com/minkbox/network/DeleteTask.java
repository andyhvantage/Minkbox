...
public class DeleteTask extends AsyncTask<String, Void, String> {
    ...
    private String TAG = com.minkbox.network.LikeTask.class.getSimpleName();
    private int status = 0;
    int product_id;
    String resultnull;


    public DeleteTask(Context context, int productid) {
        this.con = context;
    ...
    protected String doInBackground(String... params) {
        try {
            ...
            System.out.println("show all post url ---------------- " + AppConstants.DELETEPRODUCTAPI);
            HttpPost httppost = new HttpPost(AppConstants.DELETEPRODUCTAPI);
...
//                        JSONArray jsonChildArray = jsonObj.getJSONArray("result");
        ...
        try {
            if (status == 200) {
                try {
                    ...
                    int total_product= Integer.parseInt(AppPreferences.getUserproductitems(con));
                    total_product=total_product-1;
                    String total_product_value=String.valueOf(total_product);
                    AppPreferences.setUserproductitems(con,total_product_value);
                    Toast.makeText(con, "Product has been deleted successfully.", Toast.LENGTH_LONG).show();
                } catch (Exception e) {
                }
            } else if (status == 400) {
