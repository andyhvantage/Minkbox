...
import com.minkbox.ReportFeedback;
...
public class ReportUserTask extends AsyncTask<String, Void, String> {
    ...
    private String TAG = ReportTask.class.getSimpleName();
    ...
    public ReportUserTask(Context context, int customerid, String bug_st) {
        this.con = context;
        bug_status=bug_st;
    ...
    protected String doInBackground(String... params) {
        try {
            ...
            System.out.println("show all post url ---------------- " + AppConstants.REPORTUSERAPI);
            HttpPost httppost = new HttpPost(AppConstants.REPORTUSERAPI);
            jsonObj = new JSONObject();

            jsonObj.put("customer_id", AppPreferences.getCustomerId(con));
            jsonObj.put("report_user_id", customerid);
            jsonObj.put("report_status", bug_status);
    ...
    protected void onPostExecute(String result) {
        ...
        try {
            if (status == 200) {
                try {
                    Intent i = new Intent(con, ReportFeedback.class);
