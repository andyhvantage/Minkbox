...
public class LikeTask extends AsyncTask<String, Void, String> {
    ...
    private String TAG = LikeTask.class.getSimpleName();
    ...
    public LikeTask(Context context, String likest, int productid) {
        this.con = context;
        this.like_status = likest;
    ...
    protected String doInBackground(String... params) {
        try {
            ...
            System.out.println("show all post url ---------------- " + AppConstants.LIKEPRODUCTAPI);
            HttpPost httppost = new HttpPost(AppConstants.LIKEPRODUCTAPI);
            jsonObj = new JSONObject();

            jsonObj.put("customer_id", AppPreferences.getCustomerId(con));
            jsonObj.put("product_like_status",like_status);
                        ...
                        if (jsonObj.getString("status").equalsIgnoreCase("200")) {
                            ...
                            if (jsonChildArray != null && jsonChildArray.length() > 0) {
                                for (int i = 0; i < jsonChildArray.length(); i++) {
                                    ...
                                    like_status = jsonchildObj.getString("like_status");
                                    total_like = jsonchildObj.getString("totle_likes");
                                    networkFlag = "true";
                                }
    ...
    protected void onPostExecute(String result) {
        ...
        try {
            if (status == 200) {
                try {
                    System.out.println("like 0----------------+++++--------------------" + like_status);
                    if(ProductAllDetails.productAllDetailsInstance != null)
                    {
                        ProductAllDetails.productAllDetailsInstance.likeMethod(total_like);
                    }

                    if(ProductDetail.productDetailInstance != null)
                    {
                        ProductDetail.product_total_like.setText(total_like);
                    }

                    if(like_status.equalsIgnoreCase("1")){
                        ProductDetail.unLikeButton.setVisibility(View.VISIBLE);
                        ProductDetail.likeButton.setVisibility(View.GONE);
                    }
                    if(like_status.equalsIgnoreCase("0")) {
                        ProductDetail.likeButton.setVisibility(View.VISIBLE);
